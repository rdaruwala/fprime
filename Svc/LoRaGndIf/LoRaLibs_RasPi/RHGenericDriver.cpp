...
// Works correctly even on RH_millis() rollover
bool RHGenericDriver::waitAvailableTimeout(uint16_t timeout)
{
    unsigned long starttime = RH_millis();
    while ((RH_millis() - starttime) < timeout)
    {
        if (available())
	{
           return true;
...
bool RHGenericDriver::waitCAD()
{
    ...
    // 100 - 1000 ms
    // 10 sec timeout
    unsigned long t = RH_millis();
    while (isChannelActive())
    {
         if (RH_millis() - t > _cad_timeout) 
	     return false;
