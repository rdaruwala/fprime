...
PROGMEM static const RH_RF95::ModemConfig MODEM_CONFIG_TABLE[] =
{
    ...
    { 0x32,   0x64,    0x00}, // PITS SSDV (1400 bps) - HighCR4
    { 0x38,   0x64,    0x00}, // High8
    { 0x84,   0x74,    0x00}, // PITS Turbo (8000 bps)
    { 0x82,   0x64,    0x00}, // PITS TurboX (16828 bps)
    { 0x38,   0xb4,    0x00}, // PITS Telemetry(60 bps)
...
bool RH_RF95::send(std::string toSend){
  return send(toSend.c_str());
}

bool RH_RF95::send(const char * toSend){
  uint8_t data[strlen(toSend)];
  for(uint i = 0; i < strlen(toSend); i++){
    data[i] = (uint8_t) toSend[i];
  }
